...
public class LobbyUI : NetworkBehaviour
{
        ...
        for (int i = 0; i < lobbyPlayers.Count; i++)
        {
            if (lobbyPlayers[i].ClientId == serverRpcParams.Receive.SenderClientId)
            {
                lobbyPlayers[i] = new LobbyPlayerState(
                    ...
                    lobbyPlayers[i].CharacterImageNum
                    );
            }
        }
    }
    ...
    [ServerRpc(RequireOwnership = false)]
    private void ChangeCharacterImageServerRpc(int characterColorNum, ServerRpcParams serverRpcParams = default)
    {
        for (int i = 0; i < lobbyPlayers.Count; i++)
        {
            if (lobbyPlayers[i].ClientId == serverRpcParams.Receive.SenderClientId)
            {
                ...
                ServerGameNetPortal.Instance.UpdateClientPlayerData(
                    playerData.Value.PlayerName,
    ...
    private void StartGameServerRpc(ServerRpcParams serverRpcParams = default)
    {
        ...
        ServerGameNetPortal.Instance.StartGame("Level 1");
    }
    #endregion
    ...
    public void OnSelectColorClicked(int colorIndex)
    {
        // Send a message to the server to set the local client's team
        ChangeCharacterImageServerRpc(colorIndex);
    }
    #endregion
}
